...
const getTierLabel = (itemId) => {
  ...
  const enchant = itemId.includes("@") ? parseInt(itemId.split("@")[1]) : 0;
  const label = enchant > 0 ? `T${tier}.${enchant}` : `T${tier}`;
              ...
              <tr key={i}>
                <td>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    ...
                    <span
                      onClick={() => handleCopy(row.name, i)}
                      style={{ cursor: "pointer" }}
                      title="Kopyalamak için tıkla"
                    >
                      {copied === i ? (
                        <span style={{ color: "#4ade80" }}>✓ Kopyalandı</span>
                      ) : (
                        row.name
                      )}
                    </span>
